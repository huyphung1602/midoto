...
var $author$project$Midoto$PressControl = function (a) {
	return {$: 'PressControl', a: a};
};
var $author$project$Midoto$toKey = function (string) {
	var _v0 = $elm$core$String$uncons(string);
	if ((_v0.$ === 'Just') && (_v0.a.b === '')) {
		var _v1 = _v0.a;
		var _char = _v1.a;
		return $author$project$Midoto$PressCharacter(_char);
	} else {
		return $author$project$Midoto$PressControl(string);
	}
};
var $author$project$Midoto$keyDecoder = A2(
	$elm$json$Json$Decode$map,
	$author$project$Midoto$toKey,
...
var $author$project$Midoto$onKeySub = $elm$browser$Browser$Events$onKeyUp($author$project$Midoto$keyDecoder);
var $author$project$Midoto$Tick = function (a) {
	return {$: 'Tick', a: a};
};
...
var $author$project$Midoto$addToTodos = F2(
	function (input, todos) {
		...
		return _Utils_ap(
			...
			_List_fromArray(
				[
					A5($author$project$Midoto$Todo, lastIndex, input, 0, 0, $author$project$Midoto$Incomplete)
				]));
	});
...
var $author$project$Midoto$checkControlKey = F2(
	function (keyStr, isShowForm) {
		return (isShowForm && ((keyStr === 'Enter') || (keyStr === 'Escape'))) ? false : isShowForm;
	});
...
var $author$project$Midoto$deleteTodo = F2(
...
var $author$project$Ports$storeTodos = _Platform_outgoingPort('storeTodos', $elm$json$Json$Encode$string);
...
var $author$project$Midoto$todoEncoder = function (todo) {
	return $elm$json$Json$Encode$object(
		_List_fromArray(
			[
				...
				_Utils_Tuple2(
				'name',
				$elm$json$Json$Encode$string(todo.name)),
				_Utils_Tuple2(
				'workedTime',
				$elm$json$Json$Encode$float(todo.workedTime)),
				_Utils_Tuple2(
				'previousWorkedTime',
				$elm$json$Json$Encode$float(todo.workedTime)),
				_Utils_Tuple2(
...
var $author$project$Midoto$saveTodos = function (todos) {
	return $author$project$Ports$storeTodos(
		A2(
...
var $author$project$Midoto$setCompleteTodo = F3(
	function (index, completedStatus, todos) {
		return A2(
			$elm$core$List$map,
			function (todo) {
				return _Utils_eq(todo.id, index) ? _Utils_update(
					todo,
					{status: completedStatus}) : todo;
			},
			todos);
	});
...
var $author$project$Midoto$updatePreviousWorkedTime = function (todos) {
	return A2(
		$elm$core$List$map,
		function (todo) {
			return _Utils_update(
				todo,
				{previousWorkedTime: todo.workedTime});
		},
		todos);
};
...
var $author$project$Midoto$floatFromPosix = F2(
	function (zone, time) {
		return ((A2($elm$time$Time$toHour, zone, time) * 3600) + (A2($elm$time$Time$toMinute, zone, time) * 60)) + A2($elm$time$Time$toSecond, zone, time);
	});
var $author$project$Midoto$updateWorkedTime = function (model) {
	return A2(
		$elm$core$List$map,
		function (todo) {
			return _Utils_eq(todo.status, $author$project$Midoto$Active) ? _Utils_update(
				todo,
				{
					workedTime: (todo.previousWorkedTime + A2($author$project$Midoto$floatFromPosix, model.zone, model.time)) - A2($author$project$Midoto$floatFromPosix, model.zone, model.startTime)
				}) : todo;
		},
		model.todos);
};
var $author$project$Midoto$update = F2(
	function (msg, model) {
		switch (msg.$) {
			case 'PressCharacter':
				var keyChar = msg.a;
				var cmd = model.isShowForm ? $elm$core$Platform$Cmd$none : A2(
					$elm$core$Task$attempt,
					function (_v1) {
						return $author$project$Midoto$NoOp;
					},
					$elm$browser$Browser$Dom$focus('input-command-box'));
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							isShowForm: _Utils_eq(
								keyChar,
								_Utils_chr('i')) || model.isShowForm
						}),
					cmd);
			case 'PressControl':
				var keyStr = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							isShowForm: A2($author$project$Midoto$checkControlKey, keyStr, model.isShowForm)
						}),
					$elm$core$Platform$Cmd$none);
			case 'ChangeInput':
				var input = msg.a;
			...
			case 'Check':
				var index = msg.a;
				var newTodos = A3($author$project$Midoto$setCompleteTodo, index, $author$project$Midoto$Completed, model.todos);
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{inputText: '', todos: newTodos}),
			...
			case 'Uncheck':
				var index = msg.a;
				var newTodos = A3($author$project$Midoto$setCompleteTodo, index, $author$project$Midoto$Incomplete, model.todos);
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{inputText: '', todos: newTodos}),
			...
			case 'Delete':
				var index = msg.a;
				var newTodos = A2($author$project$Midoto$deleteTodo, index, model.todos);
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{inputText: '', todos: newTodos}),
			...
			case 'ActiveOn':
				var index = msg.a;
				var newTodos = A2($author$project$Midoto$setActiveTodo, index, model.todos);
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{inputText: '', todos: newTodos}),
...
var $author$project$Midoto$displayForm = function (isShowForm) {
	return isShowForm ? _List_fromArray(
		[
...
var $author$project$Midoto$onGoingTodos = function (todos) {
	return A2(
...
var $author$project$Midoto$ActiveOn = function (a) {
	return {$: 'ActiveOn', a: a};
};
var $author$project$Midoto$AddTodo = function (a) {
	return {$: 'AddTodo', a: a};
};
...
var $author$project$Midoto$getTrueIndex = F2(
	function (uiIndex, todoTuples) {
		if (!todoTuples.b) {
			return $elm$core$Maybe$Nothing;
		} else {
			if (!todoTuples.b.b) {
				var _v1 = todoTuples.a;
				var x = _v1.b;
...
var $author$project$Midoto$parseCommandUseIndex = F3(
	function (command, list, todoTuples) {
		var parseMaybeInt = function (maybeInt) {
			if (maybeInt.$ === 'Just') {
				var i = maybeInt.a;
				return command(i);
			} else {
...
var $author$project$Midoto$styleApplicationBody = _List_fromArray(
	[
		A2($elm$html$Html$Attributes$style, 'display', 'flex'),
...
var $author$project$Midoto$parseWorkingTimeToString = function (workingTime) {
	var hours = workingTime / 3600;
	var hoursRounded = hours | 0;
	var minutes = (workingTime - (hoursRounded * 3600)) / 60;
	var minutesRouded = minutes | 0;
			...
			[
				...
				_List_fromArray(
					[
						...
						'text-decoration',
						_Utils_eq(todo.status, $author$project$Midoto$Completed) ? 'line-through' : 'none'),
						A2(
						$elm$html$Html$Attributes$style,
						'font-weight',
						_Utils_eq(todo.status, $author$project$Midoto$Active) ? '600' : '300'),
						A2($elm$html$Html$Attributes$style, 'display', 'flex'),
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text(
								$elm$core$String$fromInt(uiIndex) + ('. ' + todo.name))
							])),
						A2(
						$elm$html$Html$span,
						_List_fromArray(
							[
								...
								$elm$html$Html$Attributes$style,
								'display',
								(_Utils_eq(todo.status, $author$project$Midoto$Active) || _Utils_eq(todo.status, $author$project$Midoto$Completed)) ? 'flex' : 'none')
							]),
						_List_fromArray(
							[
								$elm$html$Html$text(
								$author$project$Midoto$parseWorkingTimeToString(todo.workedTime))
							]))
					]))
			]));
};
				...
				_Utils_ap(
					_List_fromArray(
						[
							...
							A2(
								$author$project$Midoto$parseMsg,
								$author$project$Midoto$tokenize(model.inputText),
								model.todos)),
					...
					$author$project$Midoto$displayForm(model.isShowForm)),
				_List_fromArray(
					[
						A2(
						$elm$html$Html$input,
						_Utils_ap(
							_List_fromArray(
								[
									...
									$elm$html$Html$Attributes$id('input-command-box'),
									$elm$html$Html$Attributes$value(model.inputText),
									$elm$html$Html$Events$onInput($author$project$Midoto$ChangeInput)
								]),
							$author$project$Midoto$styleInputBox),
						_List_Nil)
					])),
				A2(
				$elm$html$Html$div,
				$author$project$Midoto$styleApplicationBody,
				_List_fromArray(
					[
						A2(
						$elm$html$Html$div,
						...
						A2(
							...
							A2(
								...
								_List_fromArray(
									[
										$elm$html$Html$text('On Going Tasks')
									])),
							A2(
								$elm$core$List$map,
								$author$project$Midoto$viewTodo,
